	...
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			awful.spawn.easy_async_with_shell([[playerctl status 2>/dev/null]], function(status)
				...
				if song ~= nil and song ~= " - " and song ~= "" then
					-- Get album art
					awful.spawn.easy_async_with_shell([[playerctl metadata --format '{{mpris:artUrl}}' 2>/dev/null]], function(art_url)
...
prev_button:buttons(awful.util.table.join(
...
next_button:buttons(awful.util.table.join(
	awful.button({}, 1, function()
...
music_text:buttons(awful.util.table.join(
	awful.button({}, 1, function()
