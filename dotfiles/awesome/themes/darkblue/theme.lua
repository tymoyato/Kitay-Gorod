...
local battery_widget2 = wibox.container.background(
	wibox.container.margin(
		wibox.widget({ batspr_l, bat.widget, batspr_r, layout = wibox.layout.align.horizontal }),
		0,
		0
	),
	theme.gray
...
local kbd_widget = wibox.container.background(
	wibox.container.margin(wibox.widget({ kbdwidget, layout = wibox.layout.align.horizontal }), 0, 0, 0, 0),
	theme.green
)

kbd_widget = awful.widget.keyboardlayout:new()
local kbd_widget = wibox.container.background(
	wibox.container.margin(wibox.widget({ kbd_widget, layout = wibox.layout.align.horizontal }), 0, 0, 0, 0),
	theme.green
)

-- -- Chrome_button
local chrome_button = awful.widget.button({ image = theme.chrome })
chrome_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("google-chrome-stable")
end)))
-- vivaldi_button
local vivaldi_button = awful.widget.button({ image = theme.vivaldi })
vivaldi_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("vivaldi-snapshot")
end)))
-- webstorm_button
webstorm_button = awful.widget.button({ image = theme.webstorm })
webstorm_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("webstorm")
end)))
-- idea_button
idea_button = awful.widget.button({ image = theme.idea })
idea_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("intellij-idea-ultimate-edition")
end)))
-- pycharm_button
pycharm_button = awful.widget.button({ image = theme.pycharm })
pycharm_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("pycharm")
end)))
-- teamviewer_button
teamviewer_button = awful.widget.button({ image = theme.teamviewer })
teamviewer_button:buttons(awful.util.table.join(awful.button({}, 1, function()
	awful.util.spawn("teamviewer")
end)))
...
function theme.connect(s)
	...
	-- Function to get the current track info
	local function get_track_info()
		local artist = exec("playerctl metadata artist")
		local title = exec("playerctl metadata title")
		return artist .. " - " .. title
	end

	-- Function to update the widget text
	local function update_widget()
		local track_info = get_track_info()
		music_widget:set_text(track_info)
	end

	-- Create the music widget
	music_widget = wibox.widget.textbox()
	music_widget:set_align("center")
	music_widget:set_font(beautiful.font)
	...
	local function control_player(action)
		awful.spawn.easy_async_with_shell("playerctl " .. action, function(stdout, stderr, reason, exit_code)
			if exit_code ~= 0 then
				naughty.notify({ text = "Error controlling player: " .. stderr })
			end
			update_widget()
		end)
	end

	-- Create buttons for the music widget
  ...
  -- local todo_widget = require("awesome-wm-widgets.todo-widget.todo")
	...
	s.mywibox:setup({
		...
		{ -- Right widgets
			...
			-- wibox.container.background(music_widget, theme.yellow),
			-- wibox.container.background(cmus_widget, theme.yellow),
			-- fs_widget,
			-- Temp
			w4,
			temp_widget,
